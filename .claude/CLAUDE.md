...
- Package `ghkeys`, over `filippo.io/age` + stdlib `net/http` (`golang.org/x/crypto` is a test-only dep). The HTTP transport is an injected `HTTPClient` interface, so the GitHub call is fully covered by stub clients. Failures wrap a sentinel `Error` const (`ErrFetchKeys`, `ErrNoValidKeys`) matchable with `errors.Is`.
